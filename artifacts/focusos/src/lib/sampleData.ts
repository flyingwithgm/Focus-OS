...
export function generateSampleData(name: string) {
  ...
  const tasks: Task[] = [
    { id: 't1', title: 'Complete React Assignment', priority: 'high', estMin: 120, dueAt: addDays(now, 2).toISOString(), courseId: 'c1', createdAt: now.toISOString() },
    { id: 't2', title: 'Read Chapter 4', priority: 'medium', estMin: 60, dueAt: addDays(now, 3).toISOString(), courseId: 'c2', createdAt: now.toISOString() },
  ...
  const events: Event[] = [
    { id: 'e1', type: 'class', title: 'Web Dev Lecture', courseId: 'c1', start: setMinutes(setHours(now, 10), 0).toISOString(), end: setMinutes(setHours(now, 11), 30).toISOString() },
